...
# --- PESTAÑA 3: METAS DE AHORRO / HUCHA VIRTUAL (NUEVA) ---
with tab3:
    ...
    with col_crear:
        with st.form("form_crear_meta", clear_on_submit=True):
            ...
            n_nombre = st.text_input("Nombre de la Meta (ej. Viaje a Japón, Coche nuevo)")
    ...
    # Recargamos las metas por si acaban de cambiar
    df_metas_actual = db.get_data("metas_ahorro")
    
    if not df_metas_actual.empty:
        # Mostramos las metas en tarjetas ordenadas en columnas (de 2 en 2)
        ...
        st.info("No tienes ninguna hucha activa. ¡Crea una arriba para empezar a ahorrar con propósito!")
    ...
    with col_export:
        ...
        if os.path.exists(db.DB_NAME):
            with open(db.DB_NAME, "rb") as f:
                db_bytes = f.read()
            st.download_button("Descargar Copia de Seguridad (.db)", data=db_bytes, file_name=f"backup_finanzas_{datetime.date.today()}.db", mime="application/octet-stream")
        else:
            st.warning("La base de datos aún no se ha creado.")

    with col_import:
        st.markdown("### ⬆️ Restaurar Datos")
        uploaded_file = st.file_uploader("Sube tu archivo .db", type=["db"])
